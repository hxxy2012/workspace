...
/**
 * Created by liqing on 2016/3/31.
 * 运动详情，运动累计，示范
 */
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what){
                case 0:
                    ...
                    anim = FrameAnimUtil.GetFrameAnim(ActivitySportAllDetail.this, list, Bitmap.class, false, 300);
                    if(anim!=null){
                        iv_example.setBackgroundDrawable(anim);
                        anim.start();
                    }
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                for (int i=0;i==-1?false:i<imageNumber;i++){
                    ...
                    Bitmap bitmap = HttpUtil.getSportGifImage(ActivitySportAllDetail.this, HttpConnectUtils.image_ip + sportGifImagesList.get(i).getImageUrl(), i, typeDir +"/"+ i+".png", sings);
                    if(bitmap==null){
                        i=-1;
                    }else{
                        list.add(bitmap);
