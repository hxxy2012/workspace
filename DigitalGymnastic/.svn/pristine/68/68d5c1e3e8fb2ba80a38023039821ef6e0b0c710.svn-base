...
//        SportSleepData bleData = new SportSleepData();
    ...
    MyClock myClockOne ;
    MyClock myClockTwo ;
    List mAllClock = new ArrayList<MyClock>();
    @Override
    public void onGetSettings(HashMap<String, Object> hashMap) {
        mAllClock.clear();
        myClockOne = new MyClock();
        myClockTwo = new MyClock();
        ...
        while (iterator.hasNext()) {
            ...
            switch (param.getKey()) {
                ...
                case "AlarmsSettings":
                    ...
                    a2H = alarm[4];
                    a2M = alarm[5];
                    myClockOne.setGroup_id(0);
                    myClockTwo.setGroup_id(1);
                    saveClock(myClockOne, a1H, a1M, alarm1);
                    saveClock(myClockTwo,a2H,a2M,alarm2);
                ...
                case "oneAlarmWeek":
                    ...
                    myClockOne.setWorkTime(param.getValue().toString());
                    mAllClock.add(myClockOne);
                    break;
                case "liftwrist":
                ...
                case "twoAlarmWeek":
                    ...
                    myClockTwo.setWorkTime(param.getValue().toString());
                    mAllClock.add(myClockTwo);
                    break;
            }

        }
        UtilLog.e(TAG, sb.toString());
        LocalDataUtils.saveClockData(mAllClock);
    }

    private void saveClock(MyClock pClock,int pH,int pM,int pOpen) {
        pClock.setExpanded(false);
        pClock.setHour(pH);
        pClock.setMin(pM);
        if (pOpen==1){
            pClock.setOpenorclose(true);
        }else {
            pClock.setOpenorclose(false);
        }
    }
    ...
    public void onAlarmSet() {
        ...
        EventBus.getDefault().post(new EventNotifacation(EventNotifacation.Event_clock_set_ok));
    }
    ...
    public void setAlarm(ArrayList<MyClock> pClock) {
        if (bleService != null) {
            ...
            MyClock _clockOne = pClock.get(0);
            MyClock _clockTwo = pClock.get(1);
            boolean _openOne = _clockOne.isOpenorclose();
            boolean _openTwo = _clockTwo.isOpenorclose();
            int _openOneInt = 0;
            int _openTwoInt = 0;
            if (_openOne) {
                _openOneInt = 1;
            ...
            String _oneClockWorkTime = _clockOne.getWorkTime();
            String _twoClockWorkTime = _clockTwo.getWorkTime();
            int[] _clockArray = {_openOneInt, _clockOne.getHour(), _clockOne.getMin(),_openTwoInt,_clockTwo.getHour(),_clockTwo.getMin()};
            bleService.setAlarm(_clockArray,_oneClockWorkTime,_twoClockWorkTime);
        }else{
            UtilLog.e(TAG, "ble service is null");
        }
    }
