...
//保存登陆标识
	 ...
	 * 时间先存字符串，用的时候转成数组
	 */
	public static void saveClockData(List<MyClock> clockList) {
		...
		SharedPreferences preferences = HikoDigitalgyApplication.getInstance().getSharedPreferences(
				Constants.customer, Context.MODE_PRIVATE);
		Editor editor = preferences.edit();
	...
	public static  void clearClock(){
		SharedPreferences preferences = HikoDigitalgyApplication.getInstance().getSharedPreferences(
				Constants.customer, Context.MODE_PRIVATE);
		Editor editor = preferences.edit();
		editor.putString(Constants.ClockData,"");
		editor.commit();
	}
	/**
	 * @category 获取本地闹钟信息
	...
	public static ArrayList<MyClock> getClockData(Context context) {
		...
		MyClock clock;
		ArrayList<MyClock> list = new ArrayList<MyClock>();
