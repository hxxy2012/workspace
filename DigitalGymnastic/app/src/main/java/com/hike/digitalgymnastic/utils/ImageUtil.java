...
public class ImageUtil {
	...
	public static Bitmap getResizedImage(String path, byte[] data, int target, boolean isWidth, int round) {
	...
	@SuppressWarnings("deprecation")
	public static String getTargetImage(Context context,String path, byte[] data, int target,
		...
		try {
			bm = decode(path, data, options);
			File	file = FileUtil.getCaptureFile(context);
			Utils.bitmap2JPG(bm, file);
	...
	public static String getTargetImage(Context context,File pFile,String path, byte[] data, int target,
										boolean isWidth, int round) {
		UtilLog.e(TAG,"path "+path);
		UtilLog.e(TAG,"pFile "+pFile.getPath());
		Options options = null;
		...
		try {
			bm = decode(path, data, options);
			Utils.bitmap2JPG(bm, pFile);
			filepath=pFile.getPath();
			UtilLog.e(TAG,"filepath "+filepath);
		} catch (OutOfMemoryError e) {
			L.red(e.toString());
			e.printStackTrace();
		}
